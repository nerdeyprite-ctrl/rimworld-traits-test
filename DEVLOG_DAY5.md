...
마지막으로 특성의 정확도를 높이는 데 사용한 방법
- 1. 첫번째로는 질문의 가짓수를 무식하게 늘렸음. 
  -> 결과로는 질문의 다양성이 올라가 여러번 테스트하게 하는 동기를 만들었음.
  -> 또한 일정 개수까지는 질문의 개수가 올라갈 수록 정확도가 높아졌음.
  but, 점점 개수를 올려도 정확도가 내가 원하는 수준까지 올라가지 않았음.
  and, 사용자가 점점 피로도를 느끼는 질문의 가짓수가 되어버림 (45개 + 15개) 이 이상은 올리기 버거웠음.
  2. 그래서 질문의 선택지, "대답"의 가짓수를 늘려서 해결함.
  -> 테스트 중, 선택지에 내가 원하는 대답이 없을때가 발생. 그래서 이런 상황을 줄이기 위해 선택지 증가.
  so, 결과가 더욱 정밀해지고, 선택지를 고민하는 '맛'이 생김.
  3. 그런데 전부 대답 6개로 늘리니 너무 답답해짐. 적당히 대답의 선택지를 늘리되, 너무 쓸데없이 늘리지는 않도록 수정.
